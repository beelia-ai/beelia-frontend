...
export function CoinModel() {
  ...
  useFrame((state, delta) => {
    ...
    currentRotX.current +=
      (targetRotX.current - currentRotX.current) * smoothFactor;
    currentRotY.current +=
      (targetRotY.current - currentRotY.current) * smoothFactor;
  ...
  return (
    <group>
      ...
      <pointLight position={[0, 2, 1]} intensity={1.5} color={beeliaYellow} />

      <pointLight position={[0, -1, -1]} intensity={1} color={beeliaOrange} />
    </group>
  );
}

...
export function WebGLVideo({
  ...
  // Handle user interaction to enable video playback on mobile browsers
  useEffect(() => {
  ...
  // Initialize WebGL (raw WebGL, not Three.js - avoids conflicts on iOS)
  ...
  useEffect(() => {
    if (useNativeVideo || !sourceReady) return;
  ...
  // Handle video playback for native video
  useEffect(() => {
